...
\thesisauthor{Thomas Huse}
\thesisauthorA{Philip Nyblom}
...
\nmtkeywords{English, Engelsk}
\nmtdesc{Common approach for information security management is either risk management or incident response. Root cause analysis (RCA) separates itself from these by identifying root causes and removing them. Since the approach is not often used in information security, one of our topics is to determine how well this approach works. To accomplish this, we looked at three cases about various aspects of information security.  The cases were as following: illegal filesharing at NTNU, compromised accounts at NTNU and misuse of NTNU’s resources to mine cryptocurrency. RCA is a collection of various methodologies and tools, however this report follows the methodology and tools explained in ``Root Cause Analysis: Simplified Tools and Techniques’’ \cite{RCA}. The results from the first case shows that it is largely the lack of availability that is the main reason for why students choose to download. In the case about compromised accounts we discovered that poor training and insufficient access control were the main causes. The reasons for why people chose to abuse NTNU’s resources for cryptomining can be can be attributed to the ambiguities in the IT Policy of NTNU, as well as low priority from the Digital Security Section. Following the completion of the three cases, it was concluded that the method works well, but some tools works better than others. Therefore the report also includes a framework for using RCA in information security - written based on the experience from this bachelor project.}
